...
models_resp="$(curl -fsS "${NEW_API_PUBLIC_URL}/v1/models" -H "Authorization: Bearer ${NEW_API_SERVICE_TOKEN}")"
...
chat_status="$(curl -sS -o "$chat_body" -w '%{http_code}' "${NEW_API_PUBLIC_URL}/v1/chat/completions" \
