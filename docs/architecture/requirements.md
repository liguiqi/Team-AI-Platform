...
### 场景 1：普通员工通过网页发起对话
普通员工打开 LibreChat 页面，先通过 Casdoor 完成邮箱或手机号真实认证，再在 `API-zhipu` / `API-deepseek` / `API-aliyun` / `API-kimi` / `API-doubao` 等供应商分组下选择平台允许暴露的模型，输入问题后得到对话结果。用户不持有供应商密钥，只使用平台已经开放的模型集合。
...
- `NEW-API` 必须作为唯一模型网关。
- 必须支持智谱 `ZhipuV4` 渠道。
- 必须支持 DeepSeek、阿里云百炼、Kimi OpenAI 兼容渠道与火山方舟豆包原生渠道。
...
- 必须具备 DeepSeek、阿里云百炼、Kimi 与火山方舟豆包真实联调脚本。
- 必须具备前端模型同步脚本。
- 必须具备备份和恢复脚本。
- 必须具备基础敏感信息扫描能力。
- 必须支持 systemd 开机自启动。
...
## 安全与合规要求
- `ZHIPU_API_KEY`、`DEEPSEEK_API_KEY`、`ALIYUN_API_KEY`、`KIMI_API_KEY`、`DOUBAO_API_KEY` 只允许出现在本地或生产环境变量、容器环境与 `NEW-API` 渠道配置中。
...
- `NEW-API /v1/models` 能返回当前智谱、DeepSeek、阿里云百炼、Kimi 与火山方舟豆包渠道同步后的模型集合。
