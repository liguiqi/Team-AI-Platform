...
- 智谱、DeepSeek、阿里云百炼、Kimi、火山方舟豆包与小米 MiMo 模型能按 `API-zhipu` / `API-deepseek` / `API-aliyun` / `API-kimi` / `API-doubao` / `API-mimo` 分组访问。
...
3. 如启用 DeepSeek、阿里云百炼、Kimi、火山方舟豆包或小米 MiMo，确认 `DEEPSEEK_API_KEY` / `ALIYUN_API_KEY` / `KIMI_API_KEY` / `DOUBAO_API_KEY` / `MIMO_API_KEY` 为真实可用值。
...
4. 至少验证一次邮箱真实登录
5. 至少验证一次手机号真实登录
6. 确认端点中存在 `API-zhipu`，启用 DeepSeek 时存在 `API-deepseek`，启用阿里云百炼时存在 `API-aliyun`，启用 Kimi 时存在 `API-kimi`，启用火山方舟豆包时存在 `API-doubao`，启用小米 MiMo 时存在 `API-mimo`
