...
- `API-zhipu` 只渲染 `ZHIPU_EXPOSED_MODEL`，`API-deepseek` 只渲染 `DEEPSEEK_EXPOSED_MODEL`，`API-aliyun` 只渲染 `ALIYUN_EXPOSED_MODEL`，`API-kimi` 只渲染 `KIMI_EXPOSED_MODEL`，`API-doubao` 只渲染 `DOUBAO_EXPOSED_MODEL`。
...
### 模式 1：按供应商拆分动态同步
适用场景：
- 希望 LibreChat 前端按 `API-zhipu` / `API-deepseek` / `API-aliyun` / `API-kimi` / `API-doubao` 分组展示当前供应商模型
