...
状态说明：
- 本文是历史方案文档，当前主线已经落地其中一部分内容。
- 已完成项包括：Casdoor 登录页 light / dark 自适应、登录方式收敛、LibreChat OIDC state / session 落到 Redis、旧 callback 自动重试恢复。
- 尚未落地的部分仍可作为后续二次建设参考，不应与当前主线实现混淆。
