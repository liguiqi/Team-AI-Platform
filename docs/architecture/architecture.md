...
NEW-API OpenAI 兼容接口
    -> 上游转发
智谱 ZhipuV4 / DeepSeek / 阿里云百炼 / Kimi / 火山方舟豆包 / 小米 MiMo / MiniMax / 其他未来上游
```
...
- 不直接持有智谱、DeepSeek、阿里云百炼、Kimi、火山方舟豆包、小米 MiMo、MiniMax 等上游采购密钥。
...
- 负责把 LibreChat 的标准 OpenAI 请求转发到智谱、DeepSeek、阿里云百炼、Kimi、火山方舟豆包、小米 MiMo、MiniMax 等真实上游。
...
- 默认模式：供应商拆分模式
  ...
  - LibreChat 渲染 `API-zhipu` / `API-deepseek` / `API-aliyun` / `API-kimi` / `API-doubao` / `API-mimo` / `API-minimax`
...
5. `NEW-API` 根据匹配到的渠道配置，把请求转发到智谱、DeepSeek、阿里云百炼、Kimi、火山方舟豆包、小米 MiMo、MiniMax 等真实上游。
...
- MiniMax：由 `scripts/sync-provider-models.sh` 从 MiniMax 模型 API 动态刷新，LibreChat 展示为 `API-minimax`。
...
### 上游密钥
- 变量名：`ZHIPU_API_KEY`、`DEEPSEEK_API_KEY`、`ALIYUN_API_KEY`、`KIMI_API_KEY`、`DOUBAO_API_KEY`、`MIMO_API_KEY`、`MINIMAX_API_KEY`
...
### 浏览器侧
- 不直接持有 `ZHIPU_API_KEY`、`DEEPSEEK_API_KEY`、`ALIYUN_API_KEY`、`KIMI_API_KEY`、`DOUBAO_API_KEY`、`MIMO_API_KEY`、`MINIMAX_API_KEY`
- 不直接持有短信或邮件服务密钥
