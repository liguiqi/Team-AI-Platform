...
- 渠道名称：`zhipu-primary`
- 默认测试模型：`glm-4-flash-250414`
- 运行策略：允许 `NEW-API` 渠道维护多模型矩阵，由 LibreChat 动态同步展示
...
| `glm-5-turbo` | 旗舰高速版 |
| `glm-4.7` | 高性能模型 |
| `glm-4.7-flash` | 高性能快速版 |
| `glm-4.7-flashx` | 高性能极速版 |
| `glm-4.6` | 均衡模型 |
| `glm-4.5-air` | 轻量模型 |
| `glm-4.5-airx` | 轻量增强版 |
| `glm-4.5-flash` | 超轻量模型 |
...
| `glm-4.6v` | 多模态均衡版 |
| `glm-4.6v-flash` | 多模态快速版 |
| `glm-4.1v-thinking-flashx` | 推理多模态极速版 |
| `glm-4.1v-thinking-flash` | 推理多模态快速版 |
| `glm-4v-flash` | 多模态轻量版 |
...
- 新模型上线时只需在 `ZHIPU_EXPOSED_MODEL` 中追加，无需维护映射关系。
- `NEW-API` 的 `ZhipuV4` 适配器直接处理模型名匹配。

### 模型列表更新
当智谱发布新模型时，更新 `.env` 中的 `ZHIPU_EXPOSED_MODEL` 即可：

```dotenv
ZHIPU_EXPOSED_MODEL=glm-5.1,glm-5,glm-5-turbo,...,新模型名
```
...
bootstrap 会自动同步模型矩阵到 `NEW-API` 渠道配置。
...
curl -fsS "$NEW_API_PUBLIC_URL/v1/chat/completions" \
  ...
  -d '{"model":"glm-4-flash-250414","messages":[{"role":"user","content":"回复OK即可"}],"max_tokens":10}'
```
