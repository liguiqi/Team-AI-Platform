...
- LibreChat 默认按供应商拆分为 `API-zhipu` / `API-deepseek` / `API-aliyun` / `API-kimi` / `API-doubao` / `API-mimo` / `API-minimax` 端点。
- 若配置前端白名单，则只展示白名单与 `NEW-API` 模型集合的交集。
- LibreChat 不暴露真实上游模型名，只暴露平台批准的模型名或别名。

完成标准：
- LibreChat 可访问。
- LibreChat 已识别 `API-zhipu` / `API-deepseek` / `API-aliyun` / `API-kimi` / `API-doubao` / `API-mimo` / `API-minimax` 自定义端点。
...
处理：
- 使用 `provider_prefixes()` 管理 `ZHIPU`、`DEEPSEEK`、`ALIYUN`、`KIMI`、`DOUBAO`、`MIMO`、`MINIMAX`
- LibreChat 渲染为 `API-zhipu`、`API-deepseek`、`API-aliyun`、`API-kimi`、`API-doubao`、`API-mimo`、`API-minimax`
